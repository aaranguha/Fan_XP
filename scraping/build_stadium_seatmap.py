...
<meta name="apple-mobile-web-app-title" content="Fan XP">
...
// section -- most are a clean "<num>", but plenty are "Shape<num>",
// "<num> lvl", or (accessible/companion seating with no drawn shape of its
// own) have no element at all and fall back to their base section's shape.
// Confirmed against real data (Lumen Field): plain lookup alone matched
// only 126/260 real sections; these four strategies together match 177/260
// -- the rest genuinely have no boundary drawn in TM's own artwork.
const secKeySet = new Set(Object.keys(SECS));
function findSectionEl(name){
...
function zoomToSection(name){
  ...
  document.querySelectorAll('.sec-path').forEach(el => {
    el.classList.toggle('faded', el.dataset.secName !== name);
  });
